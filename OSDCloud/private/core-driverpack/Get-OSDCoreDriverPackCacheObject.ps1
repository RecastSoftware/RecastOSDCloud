function Get-OSDCoreDriverPackCacheObject {
    ...
    Cache content inventory to search. Defaults to $global:OSDCoreCache.
    ...
    (
        ...
        [Parameter()]
        [psobject[]]
        $CacheContent = $global:OSDCoreCache
    )
    ...
    process {
        ...
        if (-not $CacheContent) {
            Write-Verbose "[$(Get-Date -format s)] [$($MyInvocation.MyCommand.Name)] OSDCoreCache is not set."
            return $null
        }
